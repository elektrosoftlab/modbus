...
func NewClient(conf *ClientConfiguration) (*ModbusClient, error) {
	...
	case "tcp+tls":
		...
		if mc.conf.TLSClientCert == nil {
			return nil, errors.New("nil client cert")
		}
		...
		if mc.conf.TLSRootCAs == nil {
			return nil, errors.New("missing CA/server certificate")
		}
	...
	default:
		if len(splitURL) != 2 {
			return nil, fmt.Errorf("missing client type in URL '%s'", mc.conf.URL)
		}
...
func (mc *ModbusClient) SetUnitId(id uint8) {
	mc.lock.Lock()
	defer mc.lock.Unlock()
	mc.unitId = id
}
...
func (mc *ModbusClient) ReadCoil(addr uint16) (bool, error) {
	...
	if len(values) < 1 {
		return false, errors.New("no values")
	}
	return values[0], nil
}
...
func (mc *ModbusClient) readBytes(addr uint16, quantity uint16, regType RegType, observeEndianness bool) (values []byte, err error) {
	...
	// (2 bytes per reg)
	regCount := uint16(quantity/2) + (quantity % 2)
